//! Provides implementation of a manually populated in-memory certificate store with serialization support
//! Provides implementation of a manually populated in-memory certificate store with serialization support.
...
//! // populate the cert_source.buffers_and_paths and cert_source.certs fields then call
//! // cert_source.index_certs(). See `populate_parsed_cert_vector` in `Pittv3` for file-system based sample.
//! // When initializing a set of partial paths, the cert_source.buffers_and_paths.partial_paths field
//! // can be empty, with population accurring via a call to find_all_partial_paths.
...
//!
use crate::error::*;
use crate::path_settings::get_time_of_interest;
use crate::pdv_utilities::{
    collect_uris_from_aia_and_sia, get_str_from_atav, is_self_issued, valid_at_time,
};
use crate::pki_environment_traits::*;
use crate::{
    pdv_certificate::*, pdv_utilities::name_to_string, ta_source::*, CertificateSource,
    CertificationPath, CertificationPathBuilder, CertificationPathSettings, ExtensionProcessing,
    NameConstraintsSet, PDVCertificate, PDVTrustAnchorChoice, PeLogLevels, PkiEnvironment,
...
use x509::{
    Certificate, GeneralName, GeneralSubtree, Name, SubjectPublicKeyInfo,
    PKIX_CE_AUTHORITY_KEY_IDENTIFIER, PKIX_CE_BASIC_CONSTRAINTS, PKIX_CE_NAME_CONSTRAINTS,
    PKIX_CE_SUBJECT_ALT_NAME, PKIX_CE_SUBJECT_KEY_IDENTIFIER,
};
...
pub struct BuffersAndPaths {
    ...
    pub partial_paths: RefCell<Vec<BTreeMap<String, Vec<Vec<usize>>>>>,
}
...
///   4. Call [`index_certs`](CertSource::index_certs) to prepare key identifier-based and name-based maps.
///
...
/// the [`BuffersAndPaths`] instance of the [`CertSource`] instance. The subsequence calls to
/// [`populate_parsed_cert_vector`](fn.populate_parsed_cert_vector.html) and
/// [`index_certs`](CertSource::index_certs) prepare the [`certs`](`CertSource`) member of the
/// [`CertSource`] structure as well as internally used name and SKID maps.
///
/// ```ignore
...
pub struct CertSource<'a> {
    ...
    skid_map: RefCell<BTreeMap<String, Vec<usize>>>,

    /// Maps certificate subject names to keys in the `certs` field.
    name_map: RefCell<BTreeMap<String, Vec<usize>>>,
}
...
impl<'a> Default for CertSource<'a> {
    ...
    /// member the index_certs method on the CertSource to prepare the instance for use.
    fn default() -> Self {
        Self::new()
    }
}
...
pub fn get_leaf_rdn(name: &Name<'_>) -> String {
    let rdn = &name[name.len() - 1];
    let r = get_str_from_atav(rdn.get(0).unwrap());
    if let Ok(s) = r {
        s
    } else {
        "unparsed".to_string()
...
fn general_subtree_to_string(pe: &PkiEnvironment<'_>, gs: &GeneralSubtree<'_>) -> String {
    match &gs.base {
        GeneralName::DirectoryName(dn) => {
            format!("DirectoryName: {}", name_to_string(pe, dn))
        }
...
impl<'a> CertSource<'a> {
    ...
    pub fn log_certs(&self, pe: &PkiEnvironment<'_>) {
        if self.certs.is_empty() {
            pe.log_message(&PeLogLevels::PeInfo, "No certificates present");
        }
    ...
    pub fn log_all_aia_and_sia(&self, pe: &PkiEnvironment<'_>, fresh_uris: &mut Vec<String>) {
        for c in self.certs.iter().flatten() {
            collect_uris_from_aia_and_sia(c, fresh_uris);
        }

        if fresh_uris.is_empty() {
            pe.log_message(&PeLogLevels::PeInfo, "No AIA or SIA URIs observed");
            return;
        }

        fresh_uris.sort();
        for u in fresh_uris {
            pe.log_message(&PeLogLevels::PeInfo, u.as_str());
        }
    }

    /// Log AIA and SIA details to PkiEnvironment's logging mechanism at debug level.
    pub fn log_all_name_constraints(&self, pe: &PkiEnvironment<'_>) {
        let mut logged_some = false;
        for (i, c) in self.certs.iter().enumerate() {
            if let Some(cert) = c {
                let nc_ext = cert.get_extension(&PKIX_CE_NAME_CONSTRAINTS);
                if let Ok(Some(PDVExtension::NameConstraints(nc))) = nc_ext {
                    ...
                    if let Some(perm) = &nc.permitted_subtrees {
                        ...
                        pe.log_message(&PeLogLevels::PeInfo, "Permitted Name Constraints");
                        for gs in perm {
                            pe.log_message(
                                &PeLogLevels::PeInfo,
                                format!("- {}", general_subtree_to_string(pe, gs)).as_str(),
                            );
                        }
                    }
                    if let Some(excl) = &nc.excluded_subtrees {
                        ...
                        pe.log_message(&PeLogLevels::PeInfo, "Excluded Name Constraints");
                        for gs in excl {
                            pe.log_message(
                                &PeLogLevels::PeInfo,
                                format!("- {}", general_subtree_to_string(pe, gs)).as_str(),
        ...
        if !logged_some {
            pe.log_message(&PeLogLevels::PeInfo, "No name constraints observed");
        }
    }

    /// Log partial path details to PkiEnvironment's logging mechanism at debug level.
    pub fn log_partial_paths(&self, pe: &PkiEnvironment<'_>) {
        if self.buffers_and_paths.partial_paths.borrow().is_empty() {
            pe.log_message(&PeLogLevels::PeInfo, "No partial paths available");
            return;
        }
            ...
            .enumerate()
        {
            ...
            for key in outer.keys() {
                ...
                for c in &self.skid_map.borrow()[key] {
                ...
                pe.log_message(&PeLogLevels::PeInfo, format!("{}: ", label).as_str());
        ...
        pe.log_message(&PeLogLevels::PeInfo, message.as_str());
    }

    /// Logs info about partial paths and corresponding buffers for a given target
    pub fn log_paths_for_target(
        &'a self,
        ...
        if self.buffers_and_paths.partial_paths.borrow().is_empty() {
            if self.certs.is_empty() {
                pe.log_message(&PeLogLevels::PeInfo, "No partial paths present");
            }
            return;
        }
        ...
        if akid_hex.is_empty() {
            ...
            for n in name_vec {
                let name_str = name_to_string(pe, n);
                if self.name_map.borrow().contains_key(&name_str) {
                    for i in &self.name_map.borrow()[&name_str] {
        ...
        if !akid_hex.is_empty() {
            let key = akid_hex;
            for outer in self.buffers_and_paths.partial_paths.borrow().iter() {
                ...
                for v in inner {
                    ...
                    if let Some(ca) = issuer {
                        if !compare_names(
                            ...
                            pe.log_message(&PeLogLevels::PeError, "Encountered CA that is likely using same SKID with different names. Skipping partial path due to name mismatch.");
                            break;
                        }
                    }
    ...
    pub fn log_paths_for_leaf_ca(
        ...
        let skid_ext = target.get_extension(&PKIX_CE_SUBJECT_KEY_IDENTIFIER);
        if let Ok(Some(PDVExtension::SubjectKeyIdentifier(skid))) = skid_ext {
            skid_hex = buffer_to_hex(skid.as_bytes());
        }
    ...
    pub fn index_certs(&self, pe: &PkiEnvironment<'_>) {
        for (i, cert) in self.certs.iter().enumerate() {
            if let Some(cert) = cert {
                let hex_skid = hex_skid_from_cert(cert);
                if self.skid_map.borrow().contains_key(&hex_skid) {
                    let mut v = self.skid_map.borrow()[&hex_skid].clone();
                    v.push(i);
                    self.skid_map.borrow_mut().insert(hex_skid, v);
                } else {
                    self.skid_map.borrow_mut().insert(hex_skid, vec![i]);
                }

                let name_str = name_to_string(pe, &cert.decoded_cert.tbs_certificate.subject);
                if self.name_map.borrow().contains_key(&name_str) {
                    let mut v = self.name_map.borrow()[&name_str].clone();
                    v.push(i);
                    self.name_map.borrow_mut().insert(name_str, v);
                } else {
                    self.name_map.borrow_mut().insert(name_str, vec![i]);
                }
            }
        }
    }
    ...
    fn get_operative_path_len_constraint(&self, path: &[usize]) -> u8 {
        ...
        for i in path {
            if let Some(ca_cert) = &self.certs[*i] {
                ...
                let pdv_ext = ca_cert.get_extension(&PKIX_CE_BASIC_CONSTRAINTS);
    ...
    pub fn check_names_in_partial_path(&self, pe: &PkiEnvironment<'_>, path: &[usize]) -> bool {
    ...
    fn find_all_partial_paths_internal(
        ...
        for (cur_cert_index, cur_cert) in self.certs.iter().enumerate() {
            ...
            if let Some(cur_cert) = cur_cert {
                ...
                if 0 == pass {
                    let ta = pe.get_trust_anchor_for_target(pe, cur_cert);
                    if let Ok(ta) = ta {
                        ...
                        if let Ok(ta_name) = ta_name {
                            ...
                            {
                                let defer_cert =
                                    DeferDecodeCertificate::from_der(cur_cert.encoded_cert);
                    ...
                    let defer_cert = DeferDecodeCertificate::from_der(cur_cert.encoded_cert);
                    if let Ok(defer_cert) = defer_cert {
                        // look for matches in map from previous row of partial_paths
                        let last_row =
                            &self.buffers_and_paths.partial_paths.borrow()[(pass - 1) as usize];

                        // get list of SKIDs for possible issuers (based on AKID and name lookups)
                        let prospective_issuers = self.find_prospective_issuers(pe, cur_cert);
                        for k in prospective_issuers {
                            //pe.log_message(&PeLogLevels::PeDebug, format!("LAST ROW FOR PASS #{} {:?}", pass, last_row.keys()).as_str());
                            ...
                            for prospective_path in prospective_paths {
                                ...
                                if let Some(prospective_ca_cert) = prospective_ca_cert {
                                    ...
                                    ) && self.check_names_in_partial_path(pe, prospective_path)
                                        && self.check_validity_in_partial_path(
                                            pe,
                                            prospective_path,
                                            cps,
                                        )
        ...
        if !new_additions.is_empty() {
            //pe.log_message(&PeLogLevels::PeDebug, format!("NEW ADDITIONS FOR PASS #{}: {:?}", pass, new_additions).as_str());
            ...
            if (PS_MAX_PATH_LENGTH_CONSTRAINT - 2) > pass {
                self.find_all_partial_paths_internal(pe, ta_vec, cps, pass + 1);
            }
        }
    }
    ...
    pub fn serialize_partial_paths(
        ...
        if CertificationPathBuilderFormats::Cbor != format {
            pe.log_message(&PeLogLevels::PeError, "Format other than CBOR requested when serializing partial paths. Only CBOR is accepted presently.");
    ...
    pub fn find_all_partial_paths(
        &self,
        pe: &'_ PkiEnvironment<'_>,
        ta_store: &dyn TrustAnchorSource,
        cps: &CertificationPathSettings<'_>,
    ) {
        let mut ta_vec = vec![];
        if let Ok(tav) = ta_store.get_trust_anchors() {
            ta_vec = tav;
        }
        self.find_all_partial_paths_internal(pe, ta_vec, cps, 0);
    }
}
...
impl CertificationPathBuilder for CertSource<'_> {
        ...
        'a: 'reference,
    {
        ...
        let ta = pe.get_trust_anchor_for_target(pe, target);
        if let Ok(ta) = ta {
        ...
        if !akid_hex.is_empty() {
            for p in self.buffers_and_paths.partial_paths.borrow().iter() {
                        ...
                        for (i, index) in indices.iter().enumerate() {
                            if let Some(cert) = &self.certs[*index] {
                                ...
                                if 0 == i {
                                    ...
                                    let ca_akid_ext =
                                        cert.get_extension(&PKIX_CE_AUTHORITY_KEY_IDENTIFIER);
                                    ...
                                    if !ta_akid_hex.is_empty() {
                                        ...
                                        if let Ok(new_ta) = pe.get_trust_anchor_for_target(pe, cert)
                                        {
                                            pe.log_message(
                                                &PeLogLevels::PeError,
...
impl CertificateSource for CertSource<'_> {
    fn get_certificates_for_skid(&'_ self, skid: &[u8]) -> Result<Vec<&PDVCertificate<'_>>> {
        ...
        if self.skid_map.borrow().contains_key(hex_skid.as_str()) {
            for i in &self.skid_map.borrow()[&hex_skid] {
                if let Some(cert) = &self.certs[*i] {
                    retval.push(cert);
                }
            }
    ...
    fn get_certificates_for_name<'a>(
        ...
        let mut retval = vec![];
        if self.name_map.borrow().contains_key(name_str.as_str()) {
            for i in &self.name_map.borrow()[&name_str] {
                if let Some(cert) = &self.certs[*i] {
                    retval.push(cert);
                }
    ...
    fn get_encoded_certificates_for_name<'a>(
        &self,
        pe: &PkiEnvironment<'a>,
        name: &Name<'a>,
    ) -> Result<Vec<Vec<u8>>> {
    ...
    for (i, cert_file) in bap.buffers.iter().enumerate() {
        if let Ok(cert) = Certificate::from_der(bap.buffers[i].bytes.as_slice()) {
            let valid = if let 0 = time_of_interest {
                ...
                let r = valid_at_time(pe, &cert.tbs_certificate, time_of_interest, false);
                if r.is_err() {
                    pe.log_message(
                        &PeLogLevels::PeError,
                        format!(
