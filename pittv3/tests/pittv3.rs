...
fn dump_cert_at_index() -> Result<(), Box<dyn std::error::Error>> {
    ...
    cmd.assert().stdout(predicate::str::contains("Encountered error while processing certificate with subject CN=DOD ID SW CA-45,OU=PKI,OU=DoD,O=U.S. Government,C=US: certificate is expired: not_after"));
...
//     cmd.assert().stdout(predicate::str::contains(
//         "Encountered error while processing certificate with subject C=US,O=U.S. Government,OU=DoD,OU=PKI,CN=DOD ID SW CA-45: certificate is expired: not_after",
//     ));
//     Ok(())
// }
