...
fn load(args: &Pittv3Args) -> Option<TaSource> {
    load_trust_anchors(&PkiEnvironment::default(), args, None).unwrap()
}
...
fn an_unreadable_store_is_an_error_rather_than_an_empty_set() {
    ...
    assert!(load_trust_anchors(&PkiEnvironment::default(), &args, None).is_err());
...
fn an_unreadable_pool_entry_fails_the_load() {
    ...
    match load_trust_anchors(&PkiEnvironment::default(), &args, None) {
        Err(e) => assert!(
