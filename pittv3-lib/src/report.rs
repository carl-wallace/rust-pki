...
pub struct PathReport {
    ...
    /// anchor and `certs[certs.len() - 1]` is the target certificate. A `PathReport` describes a
    /// path that was built, so this holds the anchor and the target at a minimum; a target for
    /// which no path was built is reported on [`TargetReport::target`] with no paths.
