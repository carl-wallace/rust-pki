...
/// [`CertificationPathSettings`] contents.
...
/// [`CertificationPathResults`] contents.
#[cfg(feature = "std_app")]
...
/// What a path's manifest says beyond the path itself: the results, the settings they were reached
/// under, what it cost, and the URI checks when the run made any.
///
/// Grouped rather than passed one by one because they travel together and always have -- every
/// caller either has all of them or none -- and because the alternative was a seven-argument
/// function growing an eighth.
#[derive(Default)]
pub struct PathLogDetails<'a> {
    /// The validation results for this path.
    pub cpr: Option<&'a CertificationPathResults>,
    /// The settings the path was judged under, after any anchor constraints were folded in.
    pub cps: Option<&'a CertificationPathSettings>,
    /// What building and validating the path cost.
    pub duration_ms: Option<u64>,
    /// The run's URI check results, when `--check-uris-when-validating` was given.
    pub uri_reports: Option<&'a UriCheckReports>,
}

/// `log_path` contributes to the manifest file related to
/// [`CertificationPath`] contents as well
...
pub fn log_path(
    ...
    if let Some(cpr) = cpr {
        ...
        render_path_manifest(pe, &mut f, path, cpr, cps, duration_ms, uri_reports);
        write_cpr_artifacts(&np, cpr);
    }
}
...
pub fn render_path_manifest(
...
) {
    {
        ...
        if let Some(reports) = uri_reports {
            render_uri_checks(f, path, reports);
        }
    }
}

/// The URI check results for every certificate on this path, closing the path's log.
///
/// **In the path's own log rather than a report beside it, and that is only honest because the
/// check runs during the run.** A manifest is an account of what happened; reachability measured
/// afterwards would be a fact about a later moment wearing the run's name. Enabled by
/// `--check-uris-when-validating`, so the fetches happen while the run does.
///
/// Certificates appear in path order -- anchor, intermediates, target -- so the section reads the
/// way the path does. One that was not checked is skipped silently rather than announced: with the
/// feature off there is no section at all, and with it on a gap means the certificate was checked
/// under another path's heading in the same run.
fn render_uri_checks(f: &mut dyn Write, path: &CertificationPath, reports: &UriCheckReports) {
    if reports.is_empty() {
        return;
    }
    ...
    let rendered: Vec<String> = certs
        .iter()
        .filter_map(|der| reports.get(der))
        .map(|r| r.to_table_string())
        .collect();
