...
//!
//! These functions are what the two GUI frontends run a validation through, and until now they had
//! no test of any kind -- `pittv3-gui-lib`'s unit tests live in `export.rs`,
//! `gui_settings_model.rs`, `retrieval.rs` and `settings_store.rs`, and `validate.rs` had none.
//! What the lower crate's suite covers is `pittv3_lib::std_utils::validate_targets`, a different
//! function reached by a different caller; every one of the four defects found on 2026-08-27 was
//! one of these two doing something the other did not.
//!
//! Material is the PKITS P-256 flavor from certval's test suite, pushed as bytes rather than read
//! through any filesystem machinery, because bytes are all a browser ever has.
